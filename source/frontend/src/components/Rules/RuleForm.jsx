...
function parseSentence(sentence) {
  const re =
    /^\s*IF\s+([^\s]+)\s*(<=|>=|=|<|>)\s*([0-9]+(?:\.[0-9]+)?)\s+THEN\s+(\S+)\s+(ON|OFF)\s*$/i;
  const match = sentence.match(re);
  if (!match) {
    throw new Error(
      "Expected: IF <source>.<metric> <operator> <value> THEN <actuator> <ON|OFF>",
    );
  }

  const [, sourceMetric, operator, threshold, actuator, state] = match;
  const parts = sourceMetric.split(".");
  if (parts.length < 2) {
    throw new Error(
      "Sensor must be in the form <source>.<metric> (e.g. rest:co2_hall.temperature)",
    );
  }

  const sensor_metric = parts.pop();
  const sensor_source = parts.join(".");
...
export default function RuleForm({ onSubmit, loading, sourceMetrics }) {
  ...
  useEffect(() => {
    ...
    if (!sensorSources.includes(form.sensor_source)) {
      ...
      setForm((prev) => ({
        ...prev,
        sensor_source: nextSource,
        sensor_metric: nextMetric,
      }));
      return;
    }
    ...
    if (!metricsForSource.includes(form.sensor_metric)) {
      setForm((prev) => ({
        ...prev,
        sensor_metric: metricsForSource[0] || "",
      }));
    }
  ...
  async function handleSubmit(e) {
    ...
    if (isSentenceMode) {
      if (!sentence.trim()) {
        setError("Please enter a rule sentence.");
        return;
      }

      try {
        payload = parseSentence(sentence);
      } catch (err) {
        setError(err.message);
        return;
      }

      payload = { ...payload, enabled: true, description: null };
    } else {
    ...
    <form className="rule-form" onSubmit={handleSubmit}>
      ...
      >
        ...
        <div>
          <button
            type="button"
            className={`btn btn--ghost btn--sm${isSentenceMode ? "" : " btn--primary"}`}
      ...
      {isSentenceMode ? (
        <div className="rule-form__row">
          ...
          <textarea
            ...
            placeholder="IF rest:co2_hall.temperature > 200 THEN cooling_fan ON"
          />
          <div
            style={{
              marginTop: "0.25rem",
          ...
          >
            Format:{" "}
            <code>
              IF source.metric &lt;operator&gt; value THEN actuator ON|OFF
            </code>
          </div>
        </div>
      ) : (
      ...
      {!isSentenceMode && (
        <>
          ...
          <div className="rule-form__row">
            ...
            <select
              name="target_actuator"
              value={form.target_actuator}
              onChange={handleChange}
            >
              {ACTUATORS.map((a) => (
      ...
      <button
        ...
        disabled={
          loading ||
          (!isSentenceMode &&
            (sensorSources.length === 0 || availableMetrics.length === 0))
        }
      >
